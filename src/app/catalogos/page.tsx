    ...
    <AppShell>
      <div className="mx-auto min-h-screen max-w-6xl px-6 py-8">
        <header className="border-b border-border pb-6">
          ...
          <p className="mt-2 text-muted-foreground">Administrá los productos disponibles para las solicitudes.</p>
        </header>
          ...
          <div className="mt-4 space-y-4">
            {(products ?? []).map((product) => (
              <div key={product.id} className="rounded-lg border border-border p-4">
                <ProductForm action={updateProduct} productId={product.id} defaultValues={{ name: product.name, presentation: product.presentation }} submitLabel="Guardar" />
