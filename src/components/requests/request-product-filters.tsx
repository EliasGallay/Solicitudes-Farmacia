...
export function RequestProductFilters({ buscar, pendiente }: { buscar?: string; pendiente?: 'con' | 'sin' }) {
  ...
  return (
    <div className="flex flex-wrap items-center gap-3">
      ...
      <ToggleGroup type="single" aria-label="Filtrar por cantidad pendiente" value={pendiente ?? ALL} onValueChange={(value) => value && setFilters({ pendiente: value === ALL ? undefined : value })}>
        {pendingFilters.map((filter) => <ToggleGroupItem key={filter.value} value={filter.value}>{filter.label}</ToggleGroupItem>)}
      </ToggleGroup>
    </div>
  )
}
